...
export function derivePackageOptions(data: ProposalData): PackageOption[] {
  ...
  return [
    buildOption({
      id: "launch-core",
      eyebrow: "Минимум для старта",
      name: "Первый запуск",
      description:
        "Фиксируем обязательный контур, чтобы команда могла стартовать без спорного объема.",
      focus: "Быстрый старт и контроль границ",
      riskControl: "Опции и дорожная карта не смешиваются с обязательным запуском",
      items: coreItems,
      isRecommended: false,
    }),
    buildOption({
      ...
      name: "Оптимальный запуск",
      description:
        "Берем обязательную часть и выбранные опции, которые усиливают ближайшую итерацию.",
      focus: "Баланс пользы, бюджета и сроков",
      riskControl: "Клиент видит, какие опции включены сверх обязательного объема",
    ...
    buildOption({
      ...
      name: "Запуск + дорожная карта",
      description:
        "Показываем полный потенциальный объем, включая опции и будущие улучшения.",
      focus: "Максимальная полнота решения",
      riskControl: "Дорожная карта явно отделена от первого запуска",
